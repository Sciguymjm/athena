...
class MenuManager:
    ...
    def pick_fox(self, state, pad):
        if self.selected_fox:
            ...
            pad.release_button(p.Button.A)
            pad.tilt_stick(p.Stick.MAIN, 0.5, 0.5)
            angle = (state.frame % 240) / 240.0 * 2 * math.pi
            pad.tilt_stick(p.Stick.C, 0.4 * math.cos(angle) + 0.5, 0.4 * math.sin(angle) + 0.5)
        else:
            ...
            if mag < 0.3:
                pad.press_button(p.Button.A)
                self.selected_fox = True
            else:
                pad.tilt_stick(p.Stick.MAIN, 0.5 * (dx / mag) + 0.5, 0.5 * (dy / mag) + 0.5)

    def press_start_lots(self, state, pad):
        if state.frame % 2 == 0:
            pad.press_button(p.Button.START)
        else:
            pad.release_button(p.Button.START)
